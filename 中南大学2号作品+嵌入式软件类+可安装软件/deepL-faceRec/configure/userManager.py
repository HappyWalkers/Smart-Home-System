    ...
    def addUser(self, name):
        ...
        if self.hasUser(name):
            ...
            if userIds == []:
                ...
                print("original newId: "+str(newId))
...
#         
    ...
    def __readCSV(self):
        print("csvFile: "+self.__CSVFile)
        self.csvIn = open(self.__CSVFile, 'rt')
    ...
    def __writeCSV(self, data):
        if type(data) is list:
            self.csvOut = open(self.__CSVFile, 'wt')
            ...
            self.csvOut = open(self.__CSVFile, 'at')
            self.writer = csv.DictWriter(self.csvOut, self.fieldNames)
            #print 'user append'
    ...
    def __readJSON(self):
        self.jsonIn = open(self.__JSONFile, 'rt')
        self.users = json.loads(self.jsonIn.read())

    def __writeJSON(self, data):
        if type(data) is list:
            self.jsonOut = open(self.__CSVFile, 'wt')
