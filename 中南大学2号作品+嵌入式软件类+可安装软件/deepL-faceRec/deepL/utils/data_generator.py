#本代码参考开源项目：https://github.com/oarriaga/face_classification/blob/master/src/utils/data_augmentation.py
import os
import time
...
class DataGenerator(object):
    def __init__(self,
        ...
        if dataset == "lfw":
        ...
        elif dataset == "olivettifaces":#对这个数据集的说明可参照此处：https://blog.csdn.net/yguoelect/article/details/52900612
            ...
            for row in range(20):#20行
                for column in range(20):#20列
                    img = olive[row * 57:(row + 1) * 57, column * 47:(column + 1) * 47]
                    data.append((img, label))#label是0-39
                    if count % 10 == 0 and count != 0:#每过10列，就是下一张人脸
                        label += 1
                    count += 1
        else:
        ...
        if data_augmentation > 0:
            ...
            data_size = len(data)
            expand_size = int(data_augmentation * data_size)#数据量扩充data_augmentation倍
            expand_list = []
        ...
        training_set_size = int(len(data) * (1 - validation_split))#训练集大小
        self.training_set = data[:training_set_size]#训练集
        self.validation_set = data[training_set_size:]#验证集
        self.keys = keys#标签定义域
